...
- `audio-workflows.md` — **AudioCraft/MusicGen** audio generation nodes (legacy — superseded by ACE Step for this user's ambient pipeline)
- `ace-step-audio.md` — **ACE Step v1.0/v1.5** native ComfyUI audio generation — node schemas, model placement, API templates, ambient prompting, 8h batch pipeline
- `ffmpeg-audio-postprocessing.md` — **FFmpeg commands** for concatenation, crossfade, normalization, and silence removal specifically for ACE Step ambient loops
- `audio-reactive-visualization.md` — **Audio-reactive image visualization** for ComfyUI — field status, known nodes (dotWaveform), and recommended hybrid pipelines for music-reactive fantasy images
- `animate-diff-vs-wan-video-img2vid.md` — comparison, VRAM requirements, debugging
- `vbox-bridged-firewall-comfyui.md` — (linked from virtualbox-guest-management skill) VM-to-host ComfyUI REST access via bridged adapter + restricted Windows firewall rule, avoiding NAT pitfalls.
...
| Template | Purpose |
|----------|---------|
| `ace-step-ambient-turbo.json` | **ACE Step v1.5 XL Turbo** ambient instrumental workflow (API format) — 120s clips, adjustable tags/lyrics/BPM/key. See `references/ace-step-audio.md` for full docs. |
...
- User wants video/audio/3D generation via AnimateDiff, Hunyuan, Wan, **ACE Step**, AudioCraft, etc.
- User asks to generate **ambient music, instrumental tracks, or long-duration audio loops**
...
## FramePack I2V (img2vid) Specifics

**See also:** `references/animate-diff-vs-wan-video-img2vid.md` — comparison,
VRAM requirements, and debugging for AnimateDiff I2V vs. Wan 2.1 I2V
workflows on 16 GB GPUs (RTX 4060 Ti).

### Installation

```bash
# Official wrapper (kijai) — use THIS one
comfy node install ComfyUI-FramePackWrapper
# Models auto-download to ComfyUI\models\diffusers\lllyasviel\FramePackI2V_HY
# Or manual: https://huggingface.co/Kijai/HunyuanVideo_comfy/blob/main/FramePackI2V_HY_fp8_e4m3fn.safetensors
```

**Do NOT use `ComfyUI-FramePackWrapper_PlusOne`** — it is a fork that may
be incompatible with the official workflow examples. If both are installed,
remove or rename the PlusOne fork.

### Workflow structure (API format)

Key nodes and their purpose:

| Node | Purpose | Key widgets |
|------|---------|-------------|
| `LoadFramePackModel` | Load transformer + quantization | `model`, `dtype`, `quantization` |
| `DualCLIPLoader` | Load text encoders (CLIP + LLaMA3) | `clip_name1`, `clip_name2`, `type=hunyuan_video` |
| `VAELoader` | Load HunyuanVideo VAE | `vae_name=hyvid\hunyuan_video_vae_bf16.safetensors` |
| `CLIPVisionLoader` | Load SigLIP vision encoder | `clip_name=sigclip_vision_384` |
| `FramePackFindNearestBucket` | Pick resolution bucket | single int (e.g. 768, 1024) |
| `FramePackSampler` | Core sampler | `steps`, `cfg`, `guidance_scale`, `seed`, `total_second_length`, `latent_window_size`, `gpu_memory_preservation` |
| `VHS_VideoCombine` | Export MP4 / WEBP | `frame_rate`, `format` |

### Prompting for FramePack

- Use **natural sentences**, not tags.
- Describe **motion in progress**: "ripples spreading", "grass swaying", "clouds drifting".
- Add **stability anchors**: "camera remains still", "fixed viewpoint", "consistent scene".
- For loops: "seamless cyclic motion", "ambient perpetual motion".
- **Negative prompt**: `static image, frozen frame, no movement, camera pan, camera zoom, abrupt changes, jitter, flickering, morphing, warping, object disappearance, scene change`.

### Critical parameters

| Parameter | Typical | Effect |
|-----------|---------|--------|
| `total_second_length` | 5.0–60.0 | Duration in seconds (×30 fps = frames) |
| `guidance_scale` | 10.0–15.0 | Higher = stronger prompt adherence |
| `latent_window_size` | 9 (default) | Frames per latent section; lower = less VRAM |
| `gpu_memory_preservation` | 6.0–12.0 | GB to keep free; higher = more CPU offload |
| `end_latent` (input pin) | Optional | If connected, interpolates toward end image; if disconnected, generates free-form motion (requires significantly more VRAM) |

### Pitfalls specific to FramePack

1. **VRAM explosion without `end_latent`** — disconnecting the `end_latent` pin
   causes the model to allocate 22–27 GB on a 16 GB card, leading to OOM.
   Keep `end_latent` connected (even with the same image) for 16 GB cards.

2. **Tensor size mismatch (`tensor a (X) must match tensor b (Y)` in RoPE)** —
   the image resolution does not align with the model's expected patch grid.
   Ensure the input image dimensions match the bucket chosen by
   `FramePackFindNearestBucket`. Common safe resolutions: 512×512, 640×360,
   768×432, 960×540. Avoid arbitrary sizes.

3. **Same image in Start + End = static video** — when `start_latent` and
   `end_latent` are identical, the model computes a zero-delta interpolation
   and produces no motion. Use a **slightly altered variant** of the same image
   in End (crop 95 %, flip horizontal, minor resize) to force motion, or
   increase `guidance_scale` to let the text prompt override the image anchor.

4. **Fork incompatibility** — workflows built for `ComfyUI-FramePackWrapper`
   (kijai) may crash with dimension errors when run under
   `ComfyUI-FramePackWrapper_PlusOne`. Use the official wrapper only.

5. **Bucket vs latent mismatch** — `FramePackFindNearestBucket` returns a
   resolution, but downstream `ImageResize+` nodes or raw image dimensions
   must match exactly. Mismatches produce `RuntimeError: Sizes of tensors must
   match` in `apply_rotary_emb` or `torch.cat`.
